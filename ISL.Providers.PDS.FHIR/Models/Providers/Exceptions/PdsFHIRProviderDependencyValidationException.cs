...
{
    /// <summary>
    /// This exception is thrown when a dependency validation error occurs while using the pds provider.
    /// For example, if an external dependency used by the provider requires data that is missing or invalid.
    /// </summary>
    public class PdsFHIRProviderDependencyValidationException
        : Xeption, IPdsProviderDependencyValidationException
    {
        public PdsFHIRProviderDependencyValidationException(
            string message,
